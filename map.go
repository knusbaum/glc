...
type atomicVal[T any] atomic.Value

func newAtomicVal[T any](e T) atomicVal[T] {
	var v atomicVal[T]
	v.Store(e)
	return v
}

func (v *atomicVal[T]) CompareAndSwap(old, new T) (swapped bool) {
	return (*atomic.Value)(v).CompareAndSwap(old, new)
}
func (v *atomicVal[T]) Load() (val T) {
	return (*atomic.Value)(v).Load().(T)
}
func (v *atomicVal[T]) Store(val T) {
	(*atomic.Value)(v).Store(val)
}
func (v *atomicVal[T]) Swap(new T) (old T) {
...
type readOnly[T comparable, U any] struct {
	m       map[T]*entry[U]
...
func newEntry[T any](i T) *entry[T] {
	return &entry[T]{p: newAtomicVal(&i)}
}
...
func (m *Map[T, U]) Load(key T) (value U, ok bool) {
	read, _ := m.read.Load().(readOnly[T, U])
	e, ok := read.m[key]
	if !ok && read.amended {
		m.mu.Lock()
...
func (e *entry[T]) load() (value T, ok bool) {
	p := e.p.Load()                //atomic.LoadPointer(&e.p)
	if p == nil || isExpunged(p) { // p == expunged {
		var ret T
		return ret, false
	}
	return *p, true
...
func (m *Map[T, U]) Store(key T, value U) {
	...
	if e, ok := read.m[key]; ok {
		...
		if !read.amended {
			...
			m.dirtyLocked()
			m.read.Store(readOnly[T, U]{m: read.m, amended: true})
		}
...
func (e *entry[T]) tryStore(i *T) bool {
	for {
		p := e.p.Load()    //atomic.LoadPointer(&unsafe.Pointer(&e.p))
		if isExpunged(p) { //p == expunged {
			return false
		}
		if e.p.CompareAndSwap(p, i) { //atomic.CompareAndSwapPointer(&e.p, p, unsafe.Pointer(i)) {
			return true
		}
	}
}
...
func (e *entry[T]) delete() (value T, ok bool) {
	for {
		...
		if e.p.CompareAndSwap(p, nil) { //atomic.CompareAndSwapPointer(&e.p, p, nil) {
			return *p, true
		}
	}
}
...
func (m *Map[T, U]) missLocked() {
	m.misses++
	if m.misses < len(m.dirty) {
		return
	}
	m.read.Store(readOnly[T, U]{m: m.dirty})
	m.dirty = nil
	m.misses = 0
}

func (m *Map[T, U]) dirtyLocked() {
	if m.dirty != nil {
		return
	}

	read, _ := m.read.Load().(readOnly[T, U])
	m.dirty = make(map[T]*entry[U], len(read.m))
	for k, e := range read.m {
...
func (e *entry[T]) tryExpungeLocked() bool {
	...
	return isExpunged(p) //p == expunged
}
